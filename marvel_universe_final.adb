...
procedure Marvel_Universe_Final is
  ...
  type Count_Matrix_Access is access Count_Matrix;

  Collaberations : Count_Matrix_Access;
...
--  end loop;
  ...
  -- Your code to do the other processing will go here.
	Collaberations := new Count_Matrix (Edges'Range(1), Edges'Range(1));
	Collaberations.all := (others => (others => 0));
	declare
		Total_Collaberations : Natural := 0;
		Collaberation_Pairs : Natural := 0;
		Collaberations_Per_Character : Float := 0.0;
		
	begin
		-- Calculate Collaberation Matrix
		for J in Edges'Range(2) loop
			for I in Edges'Range(1) loop
				if Edges(I,J) then
					for K in Edges'Range(1) loop
						if Edges(K,J) and K/=J then
							Collaberations(I,K) := Collaberations(I,K) + 1;
						end if;
					end loop;
				end if;
			end loop;
		end loop;
		-- Calculate total collaberations and collaberation pairs
		for I in Collaberations'Range(1) loop
			for J in Collaberations'Range(2) loop
				if Collaberations(I,J) > 0 then
					Total_Collaberations := Total_Collaberations + Collaberations(I,J);
					Collaberation_Pairs := Collaberation_Pairs + 1;
				end if;
			end loop;
		end loop;
		--remove duplicates
		Total_Collaberations := Total_Collaberations / 2;
		Collaberation_Pairs := Collaberation_Pairs / 2;
		--calculate the mean collaberations per character
		Collaberations_Per_Character := Float(Total_Collaberations) / Float(N_Characters);
		--print stats
		Put_Line("The total number of collaberations is " & Natural'Image(Total_Collaberations) & ".");
		Put_Line("The total number of collaberation pairs is " & Natural'Image(Collaberation_Pairs) & ".");
		Put("The mean collaberations per character is ");
		Put(Collaberations_Per_Character,Fore => 1, Aft => 2, Exp => 0);
		Put_Line(".");
	end;
		

end Marvel_Universe_Final;
